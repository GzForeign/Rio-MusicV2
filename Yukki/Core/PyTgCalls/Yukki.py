    ...
    if int(assistant) == 1:
        try:
            ...
            return await mystic.edit(
                "Failed Change Video Stream.. Please Skip Again."
            )
    elif int(assistant) == 2:
        try:
            await pytgcalls2.change_stream(
                ...
                if int(quality) == 1080:
                    ...
                    try:
                        ...
                        return await app.send_message(
                            chat_id,
                            "Error occured while switching video stream. Playout is on hold now. Please skip the stream to resume the voice chat.",
                        )
                    ...
                    if nrs == 0:
                        return await app.send_message(
                            chat_id,
                            "Failed fetch Video Formats for next stream. Please skip the stream to resume the voice chat.",
                        )
                    try:
                        await pytgclients.change_stream(
                            chat_id,
